...
class InvestmentLabService:
    ...
    def __init__(
        ...
        *,
        ...
        self.repository = repository or ValuationRepository(root / "labs" / "labs.db")
    ...
    def analyze_portfolio(
        ...
        with ThreadPoolExecutor(
            max_workers=min(len(request.markets), _LONGBRIDGE_MAX_CONCURRENCY)
        ) as executor:
            futures = {
                executor.submit(
                    self.portfolio.list_items, market, user_id=user_id, settings=settings
                ): market
        ...
        for market in request.markets:
            ...
            for item in payload.get("items", []):
                ...
                price = (
                    _number(item.get("current_price"))
                    or _number(item.get("cost_price"), 0.0)
                    or 0.0
                )
        ...
        history_symbols = list(
            dict.fromkeys(
                [str(item.get("symbol") or "") for item in holdings] + [request.benchmark_symbol]
            )
        )
        ...
        with ThreadPoolExecutor(
            max_workers=min(len(history_symbols), _LONGBRIDGE_MAX_CONCURRENCY)
        ) as executor:
            futures = {
                executor.submit(load_history, symbol): symbol
        ...
        for cash_flow in request.cash_flows:
            ...
            normalized_cash_flows.append(
                {"date": cash_flow.date, "amount": cash_flow.amount * rate}
            )
        metrics["money_weighted_irr"] = self._cash_flow_irr(normalized_cash_flows, total_value)
        metrics.update(
            {
                "concentration_hhi": round(sum(item["weight"] ** 2 for item in holdings), 6),
                "cash_weight": round(cash_value / total_value, 6),
            }
        )
        common_dates = set(portfolio_returns)
        arithmetic_contributions = {
            item["symbol"]: sum(
                item["weight"] * return_series[item["symbol"]][date] for date in common_dates
            )
        ...
        for item in holdings:
            ...
            holding_period_return = (
                self._compound(series[date] for date in sorted(common_dates))
                if series and common_dates
                else None
            )
            return_contribution = (
            ...
            contribution.append(
                {
                    ...
                    "period_return": round(holding_period_return, 6)
                    if holding_period_return is not None
                    else None,
                    "return_contribution": round(return_contribution, 6)
        ...
        metrics["contribution_residual"] = (
            round(float(period_return) - attributed_return, 6)
        ...
        exposures["thesis_risk"] = {
            key: round(value, 6)
            for key, value in sorted(thesis_risks.items(), key=lambda item: item[1], reverse=True)
        }
        return {
            "as_of": _now(),
            "methodology": "current_weight_historical_replay",
            ...
            "coverage": {
                "holdings": total_holdings,
                "valued_holdings": len(holdings),
                "excluded_fx_holdings": excluded_fx_holdings,
                "history_available": len(available_holdings),
                "history_weight": round(available_weight, 6),
            },
            "warnings": warnings,
            "limitations": [
    ...
    def _empty_portfolio_result(
        ...
        *,
        ...
        return {
            ...
            "lookback_days": request.lookback_days,
            "base_currency": request.base_currency,
            "fx_rates_used": {request.base_currency: 1.0, **request.fx_rates},
            "benchmark_symbol": request.benchmark_symbol,
            "total_value": 0,
            "equity_value": 0,
            "cash_value": 0,
            ...
            "warnings": warnings,
            "limitations": ["No analyzable portfolio data."],
        }
    ...
    def _returns_from_bars(bars: list[dict[str, Any]], symbol: str = "") -> dict[int, float]:
        ...
        ordered = sorted(
            ((int(bar.get("timestamp") or 0), _number(bar.get("close"))) for bar in bars),
    ...
    def _weighted_returns(
        holdings: list[dict[str, Any]], series: dict[str, dict[int, float]]
    ) -> dict[int, float]:
        if not holdings:
            return {}
    ...
    def _risk_metrics(
        self, portfolio: dict[int, float], benchmark: dict[int, float]
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "benchmark_correlation": round(correlation, 6) if correlation is not None else None,
            "observations": len(values),
        }

    @staticmethod
    def _exposures(
        holdings: list[dict[str, Any]], cash_by_market: dict[str, float], total: float
    ) -> dict[str, Any]:
        ...
        for item in holdings:
            ...
            country = {
                "US": "United States listing",
                "A": "Mainland China listing",
                "H": "Hong Kong listing",
            }.get(market, "Other")
            countries[country] = countries.get(country, 0) + weight
        for market, cash in cash_by_market.items():
            currencies[defaults.get(market, "UNKNOWN")] = (
                currencies.get(defaults.get(market, "UNKNOWN"), 0) + cash / total
            )
        return {
            "market": {key: round(value, 6) for key, value in markets.items()},
    ...
    @staticmethod
    def _cash_flow_irr(cash_flows: list[dict[str, Any]], terminal_value: float) -> float | None:
        if not cash_flows:
            return None
        parsed = []
        ...
        if (
            len(parsed) < 2
            or not any(amount < 0 for _, amount in parsed)
            or not any(amount > 0 for _, amount in parsed)
        ):
            return None
        ...
        def npv(rate: float) -> float:
            return sum(
                amount / ((1 + rate) ** max((date - start).total_seconds() / 31_557_600, 0))
    ...
    @staticmethod
    def _scenario(
        holdings: list[dict[str, Any]], cash_weight: float, shocks: dict[str, float]
    ) -> dict[str, Any]:
        impacts = []
        total = 0.0
        for item in holdings:
            ...
            shock = shocks.get(
                ...
                shocks.get(
                    f"market:{market}",
                    shocks.get(f"currency:{currency}", shocks.get("default", 0.0)),
                ),
            )
        ...
        return {
            "estimated_return": round(total, 6),
    ...
    @staticmethod
    def _rebalance(
        holdings: list[dict[str, Any]], targets: dict[str, float]
    ) -> list[dict[str, Any]]:
        if not targets:
            return []
        ...
        return [
            {
                "symbol": symbol,
                "current_weight": round(current.get(symbol, 0), 6),
                "target_weight": target,
                "delta_weight": round(float(target) - current.get(symbol, 0), 6),
            }
            for symbol, target in targets.items()
        ]

    def _thesis_links(self, user_id: str, holdings: list[dict[str, Any]]) -> list[dict[str, Any]]:
        latest_by_symbol = self.research.latest_theses(
            user_id, [item["symbol"] for item in holdings]
        )
        links = []
        for item in holdings:
            ...
            links.append(
                {
                    "symbol": item["symbol"],
                    "weight": round(item["weight"], 6),
                    "thesis_snapshot_id": thesis["id"] if thesis else None,
    ...
    def list_valuation_models(
        ...
        normalized = self.research.normalize_symbol(symbol) if symbol else None
        return self.repository.list_models(user_id, normalized)

    def get_valuation_model(self, user_id: str, model_id: str) -> dict[str, Any]:
        return self.repository.get_model(user_id, model_id)
    ...
    def compare_peers(self, request: PeerComparisonRequest, *, settings=None) -> dict[str, Any]:
        ...
        normalized_symbols = list(
            dict.fromkeys(self.research.normalize_symbol(symbol) for symbol in request.symbols)
        )
        ...
        with ThreadPoolExecutor(
            max_workers=min(len(normalized_symbols), _LONGBRIDGE_MAX_CONCURRENCY)
        ) as executor:
    ...
    @classmethod
    def _latest_numeric_metric(cls, value: Any) -> float | None:
    ...
    def greater_china_context(
        self, request: GreaterChinaRequest, *, settings=None
    ) -> dict[str, Any]:
        ...
        market = (
            ...
            if request.china_related_us_listing
            else "OTHER"
        )
        defaults = {
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            static_future = executor.submit(
                self.market.get_security_static_info, [symbol], settings=settings
            )
            fundamentals_future = executor.submit(
                self.fundamentals.get_security_insights, symbol, settings=settings
            )
            try:
        ...
        if request.paired_symbol:
            paired = self.compare_peers(
                PeerComparisonRequest(symbols=[symbol, request.paired_symbol]), settings=settings
            )
        return {
            "symbol": symbol,
            "market": market,
            "currency": static_info.get("currency") or defaults["currency"],
            "timezone": defaults["timezone"],
            "disclosure_languages": defaults["languages"],
            ...
            "risk_dimensions": [
                ...
                "VIE or listing structure where applicable",
                "RMB/HKD/USD currency path",
                "cross-border capital flow sensitivity",
                "controlling shareholder and related-party governance",
    ...
    def _china_checklist(market: str) -> list[str]:
        common = [
            "Reconcile Chinese and English company names and reporting currency.",
            "Check latest filings, dividends and corporate actions.",
            "Separate operating fundamentals from listing-venue and currency effects.",
        ]
        if market == "HK":
            return common + [
                "Compare HK disclosure with mainland operations and any A-share counterpart.",
                "Review liquidity, shareholder concentration and southbound-flow sensitivity.",
            ]
        if market == "A":
            return common + [
                "Review exchange board, trading-status and company-announcement context.",
                "Compare with any H-share counterpart and sector policy exposure.",
            ]
        if market == "US_CHINA":
            return common + [
                "Review listing structure, audit/disclosure jurisdiction and any HK dual listing.",
                "Reconcile ADR ratio and reporting currency before peer comparison.",
            ]
        return common
