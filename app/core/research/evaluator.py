    ...
    *,
    ...
    for event in research.list_pending_deliveries(user_id):
        try:
            ...
            research.set_alert_delivery_result(
                event["user_id"], event["id"], delivered=False, error=str(exc)
            )
...
def _observations_for_rule(
    rule: dict[str, Any], *, settings, research, market_service, fundamental_service, news_service
) -> list[dict[str, Any]]:
    symbol = rule["symbol"]
    ...
    if kind == "technical":
        ...
        payload = market_service.get_technical_indicators(
            symbol, indicators=[metric], settings=settings
        )
        ...
        return [
            {
                "value": value,
                "event_key": f"technical:{metric}:{payload.get('as_of') or value}",
                "source": {
                    "provider": "Longbridge OpenAPI",
                    "symbol": symbol,
            ...
            results.append(
                {
                    ...
                    "event_key": str(item.get("id") or item.get("url") or text),
    ...
    if kind in {"filing", "rating", "corporate_action", "valuation", "kpi"}:
        ...
        if kind in {"valuation", "kpi"}:
            metric = str(
                metadata.get("metric") or ("pe_ttm_ratio" if kind == "valuation" else "revenue")
            )
            value = _find_value(section, metric)
            return [
                {
                    "value": value,
                    "event_key": f"{kind}:{metric}:{payload.get('fetched_at') or value}",
                    "source": {
                        "provider": "Longbridge OpenAPI",
                        "symbol": symbol,
        ...
        return [
            {
                ...
                "event_key": f"portfolio:{field}:{context.get(field)}",
                "source": {"provider": "Stocks Assistant Portfolio", "symbol": symbol},
            }
        ]
    return []
...
def _deliver_event(
    research: ResearchService, rule: dict[str, Any], event: dict[str, Any], settings
) -> None:
    ...
    try:
        ...
        research.set_alert_delivery_result(
            rule["user_id"], event["id"], delivered=False, error=str(exc)
        )
