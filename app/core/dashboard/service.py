...
def _static_watchlist_row(item: dict[str, Any]) -> dict[str, Any]:
    name = (
        item.get("name") or item.get("name_cn") or item.get("name_hk") or item.get("name_en") or ""
    )
...
def _merge_quote(static_row: dict[str, Any], quote: dict[str, Any] | None) -> dict[str, Any]:
    ...
    merged["category"] = (
        quote.get("category") or static_row.get("category") or merged.get("category", "")
    )
    return merged
...
class DashboardQuoteCache:
    ...
    def set_many(
        self, user_key: str, settings_key: str, rows: list[dict[str, Any]], fetched_at: str
    ) -> None:
...
class DashboardService:
    ...
    def market(self, *, user: Any, settings: Any, mode: DashboardMode = "full") -> dict[str, Any]:
        ...
        quote_result = self._fetch_dashboard_quotes(
            ...
            category_map={
                row["symbol"]: row.get("category", "") for row in context.get("rows", [])
            },
    ...
    def portfolio(
        ...
        rows = [
            item for payload in context.get("payloads", []) for item in payload.get("items", [])
        ]
        quote_result = self._fetch_dashboard_quotes(
            user=user,
            settings=settings,
            ...
            category_map={
                canonical_symbol(row.get("symbol", "")): row.get("market", "") for row in rows
            },
            allow_remote=mode == "full" and user.can("market:read"),
        )
    ...
    def _build_market(
        self, context: dict[str, Any], quote_result: QuoteFetchResult
    ) -> dict[str, Any]:
        ...
        rows = [
            _merge_quote(row, quote_result.quotes.get(row["symbol"]))
    ...
    def _build_watchlist(
        ...
        *,
        ...
        return self._module_meta(
            ...
            items=rows,
            views=views,
            counts_by_category=context.get(
                "counts", {category: 0 for category in WATCHLIST_CATEGORIES}
            ),
            total=len(rows),
    ...
    def _portfolio_context(self, *, user: Any) -> dict[str, Any]:
        if not user.can("portfolio:read"):
            return {
                "available": False,
                "error": "Missing permission: portfolio:read",
                "payloads": [],
            }
    ...
    def _build_portfolio(
        self, context: dict[str, Any], quote_result: QuoteFetchResult, *, user: Any
    ) -> dict[str, Any]:
        if not context.get("available", True):
            return self._module_meta(
                available=False,
