...
class GetResearchContextTool(BaseTool):
    ...
    def __init__(self, *, research_service=None, user_id: str | None = None):
    ...
    def execute(self, args: dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            summary["documents_list"] = self.research_service.list_documents(
                self.user_id or "", symbol
            )
            source = source_reference(
                source_type="local_research_workspace",
                provider="Stocks Assistant Research",
                title=f"{symbol} research workspace",
                symbol=symbol,
                locator="latest Thesis, evidence, decisions, documents and alerts",
            )
            return ToolResult.success(
                summary, ext_data=evidence_metadata([evidence_for_source(source)])
            )
