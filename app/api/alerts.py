...
def delete_rule(
    rule_id: str, current_user: CurrentUser = Depends(require_permissions("scheduler:write"))
):
    try:
...
@router.post("/rules/{rule_id}/evaluate", response_model=AlertEventResponse | None)
def evaluate_rule(
    rule_id: str,
    body: AlertEvaluationRequest,
    current_user: CurrentUser = Depends(require_permissions("scheduler:run")),
):
    try:
...
def list_events(
    ...
    return get_research_service().list_alert_events(
        current_user.id, symbol=symbol, status=status, limit=limit
    )
...
def retry_event(
    event_id: str, current_user: CurrentUser = Depends(require_permissions("scheduler:write"))
):
    try:
