...
class DelegateAgentToolTest(unittest.TestCase):
    ...
    def test_allow_all_mcp_tools_expands_current_and_future_mcp_tools(self):
        roles = {
            "mcp_researcher": {
                "description": "MCP Research",
                "system_prompt": "Research.",
                "tool_allowlist": ["web_fetch"],
                "max_steps": 4,
                "allow_dangerous_tools": False,
        ...
        result, events = self.run_tool(
            ...
            settings=fake_settings(multi_agent_roles=roles),
            parent=parent,
        )

        self.assertEqual(result.status, "success")
        start_event = next(event for event in events if event["type"] == "subagent_start")
        self.assertEqual(
            start_event["data"]["tools"],
            ["web_fetch", "mcp_alpha_search", "mcp_beta_read"],
        )

        result, _events = self.run_tool(
            {"tasks": [{"role": "mcp_researcher", "task": "test", "tools": ["mcp_beta_read"]}]},
            settings=fake_settings(multi_agent_roles=roles),
            parent=parent,
        )
        self.assertEqual(result.status, "success")
        ...
        result, _events = self.run_tool(
            {"tasks": [{"role": "mcp_researcher", "task": "test", "tools": ["bash"]}]},
            settings=fake_settings(multi_agent_roles=roles),
            parent=parent,
        )
        self.assertEqual(result.status, "error")
