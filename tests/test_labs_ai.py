...
from app.core.labs.ai_service import LabAIService, LabAIRuntime, LabAIRunStore
...
def fake_user(user_id="user-1", permissions=None):
    return CurrentUser(user_id, user_id, user_id, (), frozenset(permissions if permissions is not None else {
        "chat:write", "portfolio:read", "fundamentals:read", "market:read", "knowledge:write",
    }), True)
...
class LabFakePortfolio:
    ...
    def list_items(self, market, user_id=None, settings=None):
        ...
        symbol, currency, price = {"US": ("AAA.US", "USD", 120), "H": ("00700.HK", "HKD", 400), "A": ("600519.SH", "CNY", 1500)}[market]
        return {"market": market, "total_capital": "1000", "items": [{
            "symbol": symbol, "shares": "10", "cost_price": str(price * .8),
            "current_price": str(price), "stock_value": str(price * 10), "currency": currency,
        }]}


class LabFakeMarket:
    def get_candlesticks(self, symbol, period, count, settings=None):
        return {"bars": [{"timestamp": 1_700_000_000 + day * 86400, "close": 100 + day + (day % 3)} for day in range(45)]}

    def get_security_static_info(self, symbols, settings=None):
        return [{"symbol": symbol, "currency": "HKD" if symbol.endswith(".HK") else "CNY" if symbol.endswith((".SH", ".SZ")) else "USD", "lot_size": 100} for symbol in symbols]

    def get_realtime_quotes(self, symbols, settings=None):
        return {"source": "Fake Longbridge quote fixture", "symbols": symbols, "quotes": [{"symbol": symbol, "last_done": "150", "timestamp": "2026-09-09T00:00:00Z"} for symbol in symbols]}


class LabFakeFundamentals:
    def get_security_insights(self, symbol, settings=None):
        return {"symbol": symbol, "source": "Fake Longbridge fixture", "fetched_at": "2026-09-09T00:00:00Z",
                "company": {"name": symbol, "currency": "USD"}, "valuation": {"pe_ttm_ratio": 20 if symbol == "AAA.US" else 30, "pb_ratio": 2}, "filings": []}

    def get_financial_reports(self, symbol, settings=None):
        return {"symbol": symbol, "source": "Fake financial statement fixture", "period": "FY2025", "currency": "USD", "unit": "ones",
                "revenue": 1_000_000, "shares_outstanding": 100_000, "cash": 200_000, "debt": 100_000, "net_income": 150_000}


def make_lab_service(workspace):
    return InvestmentLabService(workspace, portfolio_service=LabFakePortfolio(), market_service=LabFakeMarket(),
                                fundamental_service=LabFakeFundamentals(), research_service=ResearchService(workspace))


def dcf_args(artifact_id="evidence_2"):
    assumptions = {"revenue": 1_000_000, "shares_outstanding": 100_000, "cash": 200_000, "debt": 100_000,
                   "fcf_margin": .15, "revenue_growth": .05, "wacc": .1, "terminal_growth": .02, "years": 5}
    evidence = {}
    for field in assumptions:
        if field in {"revenue", "shares_outstanding", "cash", "debt"}:
            evidence[field] = {"kind": "source", "artifact_id": artifact_id, "path": "/" + field, "note": "FY2025 USD amounts in single units; shares in single shares"}
        else:
            evidence[field] = {"kind": "assumption", "note": "Explicit illustrative research assumption, not a forecast"}
    return {"symbol": "AAA.US", "title": "AAA.US 基准情景", "currency": "USD", "model_type": "dcf", "assumptions": assumptions, "evidence": evidence}
...
class LabFakeProvider:
    ...
    def call_stream(self, request):
        ...
        if self.tool_args is not None and len(self.requests) == 1:
            yield {"choices": [{"delta": {"content": "正在核对估值假设。", "tool_calls": [{"index": 0, "id": "fixture_calc", "function": {
                "name": "calculate_lab_valuation", "arguments": json.dumps(self.tool_args),
            }}]}}]}
        else:
...
class LabsAITest(unittest.TestCase):
    def setUp(self):
        ...
        self.settings = Settings(workspace_dir=self.tmp.name, llm_api_key="fake-secret", llm_model="fake-only")
        self.user = fake_user()
    ...
    def collect(self, request, provider, user=None):
        async def run():
            return [event async for event in self.ai.stream(user or self.user, request, self.settings, provider)]
        return asyncio.run(run())

    def runtime(self, lab="valuation"):
        request = LabAIRequest(lab=lab, objective="Analyze the available evidence", symbols=["AAA.US"] if lab != "portfolio" else [])
        store = LabAIRunStore(self.lab)
        run = store.create(self.user.id, request)
        return LabAIRuntime(self.lab, store, self.user, request, self.settings, run, lambda *_: None)

    def test_real_agent_collects_current_user_portfolios_with_explicit_native_currency_stress(self):
        provider = LabFakeProvider()
        events = self.collect(LabAIRequest(lab="portfolio", objective="检查集中度和压力风险"), provider)
        ...
        self.assertEqual([item["data"]["base_currency"] for item in run["artifacts"]], ["USD", "CNY", "HKD"])
        self.assertTrue(all(item["data"]["scenario"]["estimated_return"] < 0 for item in run["artifacts"]))
        self.assertTrue(all(item["data"]["scenario_assumptions"]["hypothetical"] for item in run["artifacts"]))
        self.assertIn("跨币种", " ".join(run["warnings"]))
        self.assertEqual([tool["name"] for tool in provider.requests[0].tools], ["get_lab_data"])
        self.assertEqual(provider.requests[0].tools[0]["parameters"]["properties"]["action"]["enum"], ["portfolio"])
    ...
    def test_real_agent_calculates_and_atomically_saves_evidenced_valuation(self):
        ...
        events = self.collect(LabAIRequest(lab="valuation", symbols=["AAA.US"], objective="估值并注明假设"), provider)
        run = events[-1]["data"]["run"]
        self.assertEqual(run["status"], "completed")
        ...
        self.assertEqual(len([event for event in events if event["type"] == "message_delta" and event["data"].get("reset")]), 2)
    ...
    def test_valuation_without_knowledge_write_retains_calculation_but_does_not_save_model(self):
        ...
        events = self.collect(LabAIRequest(lab="valuation", symbols=["AAA.US"], objective="估值"), LabFakeProvider(tool_args=dcf_args()), user)
        run = events[-1]["data"]["run"]
        self.assertEqual(run["status"], "completed")
        self.assertTrue(any(item["kind"] == "valuation" for item in run["artifacts"]))
        self.assertEqual(self.lab.list_valuation_models(user.id), [])

    def test_calculation_rejects_invented_facts_missing_debt_foreign_sources_and_nonfinite_values(self):
        ...
        self.assertEqual(len([item for item in runtime.run["artifacts"] if item["kind"] == "valuation"]), 1)
    ...
    def test_peer_median_cannot_reference_an_individual_row_or_target_company(self):
        ...
        args = {"symbol": "CCC.US", "title": "Peers", "currency": "USD", "model_type": "relative",
                "assumptions": {"metric": "pe_ttm_ratio", "peer_median": 20, "target_metric": 150_000},
                "evidence": {"peer_median": {"kind": "source", "artifact_id": peer["id"], "path": "/rows/0/metrics/pe_ttm_ratio", "note": "wrong single row"},
                             "target_metric": {"kind": "source", "artifact_id": source["id"], "path": "/net_income", "note": "annual total USD net income"}}}
    ...
    def test_data_actions_cannot_read_other_users_or_bypass_lab_scope(self):
        ...
        self.assertEqual(runtime.data_tool({"action": "portfolio", "user_id": "victim"}).status, "error")
        self.assertEqual(runtime.data_tool({"action": "financial_reports", "symbol": "AAA.US"}).status, "error")
    ...
    def test_bootstrap_failure_returns_report_with_explicit_data_gap(self):
        with patch.object(self.lab.fundamentals, "get_financial_reports", side_effect=RuntimeError("SDK fake-secret")):
            events = self.collect(LabAIRequest(lab="valuation", objective="研究AAA.US", symbols=["AAA.US"]), LabFakeProvider())
    ...
    def test_provider_failure_is_sanitized_and_persisted(self):
        events = self.collect(LabAIRequest(lab="valuation", objective="研究AAA.US"), LabFakeProvider(failure=True))
        run = events[-1]["data"]["run"]
    ...
    def test_disconnect_marks_canceled_and_late_model_response_cannot_complete(self):
        ...
        async def run():
            stream = self.ai.stream(self.user, LabAIRequest(lab="valuation", objective="研究AAA.US"), self.settings, provider)
    ...
    def test_timeout_is_terminal_and_releases_worker_after_blocked_call_finishes(self):
        ...
        self.ai.timeout_seconds = .05
        events = self.collect(LabAIRequest(lab="valuation", objective="研究AAA.US"), LabFakeProvider(block=release))
        run = events[-1]["data"]["run"]
        self.assertEqual(run["status"], "failed")
    ...
    def test_atomic_completion_rolls_back_models_when_canceled_before_commit(self):
        ...
        def cancel_after_insert(*args, **kwargs):
            ...
            runtime.cancel.set()
            return result
        with patch.object(self.lab, "create_valuation_model", side_effect=cancel_after_insert):
    ...
    def test_cancel_during_database_commit_does_not_block_event_loop_and_rolls_back(self):
        ...
        async def run():
            commit = asyncio.create_task(asyncio.to_thread(self.ai.store.complete, runtime, "report"))
            self.assertTrue(await asyncio.to_thread(inserted.wait, 1))
            runtime.stop("failed", "fixture timeout")
            finish = asyncio.create_task(asyncio.to_thread(runtime.finish, "failed", "fixture timeout"))
            try:
    ...
    def test_request_and_scenario_validation(self):
        for request in ({"lab": "valuation", "objective": "  "}, {"lab": "portfolio", "objective": "x", "user_id": "victim"}):
            with self.assertRaises(ValidationError):
                LabAIRequest(**request)
        for shocks in ({"default": -.1}, {"default": float("nan")}, {"default": -2}):
            with self.assertRaises(ValidationError):
    ...
    def test_api_enforces_permissions_user_history_and_preserves_legacy_routes(self):
        ...
        with patch.object(labs_api, "get_investment_lab_service", return_value=self.lab), \
             patch.object(labs_api, "get_effective_settings", return_value=self.settings), \
             patch("app.deps.create_llm_provider", return_value=LabFakeProvider()) as provider_factory, TestClient(app) as client:
            selected[0] = fake_user(permissions={"chat:write"})
            response = client.post("/api/v1/labs/ai/stream", json={"lab": "portfolio", "objective": "review"})
            self.assertEqual(response.status_code, 403)
            provider_factory.assert_not_called()
            selected[0] = self.user
            response = client.post("/api/v1/labs/ai/stream", json={"lab": "greater_china", "objective": "研究700.HK", "symbols": ["700.HK"]})
            ...
            selected[0] = self.user
            self.assertEqual(client.post("/api/v1/labs/portfolio/analyze", json={"markets": ["US"]}).status_code, 200)
