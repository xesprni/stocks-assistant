...
class Phase1ResearchServiceTest(unittest.TestCase):
    ...
    def test_thesis_versions_capture_changes_and_user_isolation(self):
        first = self.service.create_thesis(
            ...
            "aapl.us",
            ThesisSnapshotCreate(
                payload=ThesisPayload(business_model="Hardware", confidence=0.5),
        ...
        self.assertEqual(
            ["business_model", "confidence"], second["change_summary"]["changed_fields"]
        )
    ...
    def test_document_versions_are_deduplicated_and_have_page_locators(self):
        first = self.service.ingest_document(
            ...
            ResearchDocumentCreate(
                ...
                page_texts=["Revenue grew", "Margin stable"],
        ...
        second = self.service.ingest_document(
            ...
            ResearchDocumentCreate(
                ...
                page_texts=["Revenue grew faster", "Margin expanded"],
            ),
        )
    ...
    def test_alert_event_is_deduplicated_and_contains_portfolio_and_thesis_context(self):
        self.portfolio.add_item(
            PortfolioItemCreate(
                market="US", symbol="AAPL.US", name="Apple", shares="10", cost_price="150"
            ),
            user_id="user-1",
        )
        thesis = self.service.create_thesis(
            ...
            "AAPL.US",
            ThesisSnapshotCreate(
                payload=ThesisPayload(invalidation_conditions=["Price below 100"], confidence=0.8),
    ...
    def test_external_delivery_can_be_queued_and_retried(self):
        ...
        event = self.service.record_evaluation(
            "user-1", rule["id"], observed_value=210, event_key="quote-delivery"
        )
        ...
        self.service.set_alert_delivery_result(
            "user-1", event["id"], delivered=False, error="network"
        )
    ...
    def test_decision_log_can_record_outcome(self):
        decision = self.service.create_decision(
            ...
            "AAPL.US",
            DecisionCreate(
                action="Hold", rationale="Evidence remains intact", evidence_ids=["ev_1"]
            ),
        )
        reviewed = self.service.update_decision_outcome(
            "user-1", decision["id"], "Thesis confirmed after earnings"
        )
    ...
    def test_cross_symbol_thesis_links_are_rejected(self):
        ...
        with self.assertRaisesRegex(ValueError, "different symbol"):
            self.service.create_decision(
                ...
                "AAPL.US",
                DecisionCreate(
                    action="Hold", rationale="Wrong link", thesis_snapshot_id=thesis["id"]
                ),
            )
        with self.assertRaisesRegex(ValueError, "different symbol"):
        ...
        rule = self.service.create_alert_rule(
            "user-1",
            AlertRuleCreate(
                symbol="AAPL.US", name="Valid", condition_type="price", operator="gt", threshold=200
            ),
        )
        with self.assertRaisesRegex(ValueError, "different symbol"):
    ...
    def test_research_metrics_are_computable_from_audited_records(self):
        self.service.create_thesis(
            "user-1",
            "AAPL.US",
            ThesisSnapshotCreate(payload=ThesisPayload(confidence=0.6), reason="Baseline"),
        )
        rule = self.service.create_alert_rule(
            "user-1",
            AlertRuleCreate(
        ...
        event = self.service.record_evaluation(
            "user-1", rule["id"], observed_value=210, event_key="metrics-event"
        )
    ...
    def test_saved_evidence_and_materialized_document_feed_research_context(self):
        ...
        evidence = self.service.save_evidence(
            ...
            ResearchEvidenceCreate(
                source_id="src_1",
                source={"title": "Filing", "url": "https://example.com/filing"},
                relation="supports",
            ),
        )
    ...
    def test_live_price_evaluation_creates_event(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = evaluate_due_alerts(
                ...
                fundamental_service=_Fundamentals(),
                news_service=_News(),
                user_id="user-1",
                force=True,
            )
