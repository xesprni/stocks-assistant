...
def test_oauth_configuration_uses_sdk_handle_and_preserves_endpoints_and_language():
    ...
    settings = Settings(longbridge_auth_mode="oauth", longbridge_oauth_client_id="client-a", app_language="en", longbridge_http_url="https://openapi.longbridge.com")
    handle = object()
    with patch("app.core.market.longbridge_oauth.get_oauth", return_value=handle), patch("longbridge.openapi.Config") as config:
        longbridge_config(settings)
        config.from_oauth.assert_called_once_with(handle, http_url="https://openapi.longbridge.com", quote_ws_url=None, language=Language.EN)
...
def test_oauth_does_not_fall_back_to_a_different_apikey_account():
    ...
    settings = Settings(longbridge_auth_mode="oauth", longbridge_app_key="legacy-key", longbridge_app_secret="legacy-secret", longbridge_access_token="legacy-token")
    with patch("app.core.market.longbridge_oauth.get_oauth", side_effect=LongbridgeUnavailableError("please authorize")), patch("longbridge.openapi.Config") as config:
...
def test_oauth_account_and_mode_partition_context_cache():
    ...
    assert len({credential_signature(first), credential_signature(second), credential_signature(legacy)}) == 3
...
def test_readiness_honors_selected_authentication_and_response_hides_inherited_id():
    ...
    with patch("app.api.config.oauth_connected", return_value=True):
        assert next(check for check in _readiness_checks(settings) if check.component == "longbridge").configured
...
def test_readiness_does_not_treat_legacy_keys_as_an_oauth_connection():
    settings = Settings(longbridge_auth_mode="oauth", longbridge_app_key="key", longbridge_app_secret="secret", longbridge_access_token="token")
    with patch("app.api.config.oauth_connected", return_value=False):
        assert not next(check for check in _readiness_checks(settings) if check.component == "longbridge").configured
...
def test_personal_credentials_do_not_silently_switch_to_a_system_oauth_account():
    ...
    store = MagicMock()
    store.get_config.return_value = {"longbridge_auth_mode": "oauth", "longbridge_oauth_client_id": "system-client"}
    cases = [
        ({}, "oauth", "system-client"),
        ({"longbridge_app_key": "personal-legacy"}, "apikey", ""),
        ({"longbridge_auth_mode": "oauth"}, "oauth", ""),
        ({"longbridge_auth_mode": "oauth", "longbridge_oauth_client_id": "personal-client"}, "oauth", "personal-client"),
    ]
    try:
