  ...
  return (
    <section className="panel motion-panel page-enter flex min-h-0 min-w-0 flex-1 flex-col rounded-md lg:h-full">
      ...
      <div className="panel-body min-h-0 flex-1 lg:overflow-y-auto">
        ...
        <form className="mb-3 flex flex-col gap-2 sm:flex-row" onSubmit={(e) => { e.preventDefault(); handleSearch(); }}>
            ...
            {files.map((f) => (
              <div key={f.path}>
                <div className={cn("flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-sm transition-colors hover:bg-muted/60", expandedPath === f.path && "bg-muted/60")}>
                  <button
                    type="button"
                    className="flex min-w-0 flex-1 flex-wrap items-center gap-2 text-left sm:flex-nowrap"
