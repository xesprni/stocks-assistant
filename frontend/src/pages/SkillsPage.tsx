          ...
          <div className="min-w-0 space-y-3 overflow-hidden">
            <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div className="min-w-0">
                ...
                <p className="break-words text-xs text-muted-foreground">{copy.localSubtitle}</p>
              </div>
            </div>
            ...
            {isLoading ? (
              ...
              <div className="grid min-w-0 gap-2 xl:grid-cols-2">
                {skills.map((skill) => (
                  <div
                    key={skill.name}
                    className="message-bubble min-w-0 overflow-hidden rounded-md border border-border/80 bg-card/80 p-3 transition-colors hover:border-primary/50"
                  >
                    <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div className="min-w-0 flex-1">
                        <div className="flex min-w-0 flex-wrap items-center gap-1.5 sm:gap-2">
                          <span className="min-w-0 max-w-full break-all text-sm font-semibold sm:truncate">{skill.name}</span>
                        ...
                        {skill.description ? <p className="mt-1 break-words text-xs leading-5 text-muted-foreground">{skill.description}</p> : null}
                        ...
                        {skill.file_path ? (
                          <p className="mt-1 max-w-full overflow-hidden text-ellipsis whitespace-nowrap font-mono text-[10px] text-muted-foreground/60">{skill.file_path}</p>
                        ) : null}
                      </div>
