...
export function UsersPage({ language }: { language: AppLanguage }) {
  ...
  async function load() {
    ...
    try {
      ...
      setPagePermissions(roleResponse.page_permissions ?? {});
  ...
  async function handleSavePagePermission(page: string, permission: string) {
    setError("");
    setNotice("");
    setPagePermissions((current) => ({ ...current, [page]: permission }));
    try {
      const response = await savePagePermission(page, { permission });
      setPagePermissions(response.page_permissions ?? {});
      setNotice(t.pagePermissionSaved);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : t.pagePermissionSaveFailed);
      void load();
    }
  }
              ...
              <div className="space-y-3">
                <div className="rounded-md border border-border/80 bg-background/60 p-3">
                  <div className="mb-3 flex items-center gap-2">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-semibold">{t.pagePermissionManagement}</p>
                      <p className="mt-1 text-xs text-muted-foreground">{t.pagePermissionHint}</p>
                    </div>
                  </div>
