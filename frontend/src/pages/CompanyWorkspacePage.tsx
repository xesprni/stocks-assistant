...
import { BarChart3, Bell, BookOpen, Bot, BriefcaseBusiness, Building2, Calculator, ExternalLink, FileText, Loader2, Plus, RefreshCw, Save, ScrollText, Sparkles } from "lucide-react";
...
import {
  createResearchDecision, createThesisSnapshot, getCandlesticks,
  getDashboardSymbolInsights, getSecurityWorkspaceSummary,
  listResearchDecisions, listResearchEvidence, listThesisSnapshots,
  saveResearchEvidence, updateResearchDecision,
} from "@/lib/api";
import type { AppLanguage } from "@/lib/i18n";
import type { CandlestickItem, DashboardSymbolInsightsResponse, ResearchDecision, ResearchEvidence, SecurityWorkspaceSummary, ThesisPayload, ThesisSnapshot } from "@/types/app";
  ...
  return (
    <section className="panel motion-panel page-enter flex min-h-0 min-w-0 flex-1 flex-col rounded-md lg:h-full">
      ...
      <div className="panel-body min-h-0 flex-1 lg:overflow-y-auto">
        ...
        {tab === "documents" ? <ResearchDocumentsTab key={symbol} language={language} onChanged={loadSummary} symbol={symbol} /> : null}
